...
void Motor_MoveSideways(bool isRight);
...
void setup() {  
  ...
  Motor_MoveSideways(true); ///Move right
  delay(1000);
  Motor_MoveSideways(false); ///Move left
  delay(1000);
...
void Motor_MoveSideways(bool isRight) {
  bool direction[4] = { 1, 1, 0, 0};///forward, left 1, right 0
  for (uint8_t i = 0; i < 4; i++) {
    digitalWrite(motordirectionPin[i], (isRight == direction[i]) ? HIGH : LOW);
    analogWrite(motorpwmPin[i], 125); // Set speed to 125
  }
  Serial.println(isRight ? "Move right" : "Move left");
}
