...
void setup() {
  ...
  Rgb_Show(255,0,0);                // set the RGB light to red
  ...
  Rgb_Show(0,0,255);                // set the RGB light to blue
  ...
  Rgb_Show(0,255,0);                // set the RGB light to green
  delay(1000);
...
void loop() {
  ...
  // Listen to Serial Monitor for LED color commands
  if (Serial.available() > 0) {
    String cmd = Serial.readStringUntil('$');
    cmd.trim();
    // Beep to indicate command received
    tone(buzzerPin, 1200);
    delay(100);
    noTone(buzzerPin);
    ...
    if (sepIndex > 0) {
      tone(buzzerPin, 1200);
      delay(100);
      String mainCmd = cmd.substring(0, sepIndex);
      String paramStr = cmd.substring(sepIndex + 1);
      int paramSep = paramStr.indexOf('|');
      String param = paramSep >= 0 ? paramStr.substring(0, paramSep) : paramStr;
      // Use switch for main command
      switch (mainCmd.charAt(0)) {
        case 'A':
          if (param == "1") {
 ...
 * @param angle   Controls the movement direction of the car, with 0 degrees being the direction of the front of the car, and counter-clockwise as the positive direction. Ranging from 0 to 359
 * @param velocity   Controls the speed of the car, ranging from 0 to 100
 * @param rot     Controls the rotational speed of the car, ranging from -100 to 100. If it is greater than 0, the car has a rotational speed in counter-clockwise direction; if it is less than 0, the car has a rotational speed in clockwise direction
